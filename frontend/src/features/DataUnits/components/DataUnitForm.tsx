...
const DataUnitForm: React.FC = () => {  
  ...
  if (!model) return <p>No DataUnit details available.</p>;
    ...
    const saveData = async (e: React.FormEvent) => { 
      ...
      if (DataUnitId) {
        try {
          const updateDataUnitRequest: UpdateDataUnitRequest = {
            id: DataUnitId,
          ...
          const dataUnitService = new DataUnitService();
          const response: ResponseResult<GetDataUnitsDataUnit> = await dataUnitService.updateDataUnit({
            ...updateDataUnitRequest,
      ...
      {
        try {
          const createDataUnitRequest: CreateDataUnitRequest = {
            NameUnit: model.NameUnit,
            Plan: model.Plan || '',
            cancelToken: source.token
          };
          ...
          const dataUnitService = new DataUnitService();
          const response: ResponseResult<GetDataUnitsDataUnit> = await dataUnitService.createDataUnit({
            ...createDataUnitRequest, 
            cancelToken: source.token, });
      ...
      <section className="flex w-full items-center bg-gray-50 dark:bg-gray-900">
        <div className="w-full">
          <div className="relative bg-white shadow-md sm:rounded-lg dark:bg-gray-800">
            ...
            <Card>  
            <form className="mx-1 flex flex-col gap-4">
              <div>
